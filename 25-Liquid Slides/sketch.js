...
class SliderLoadingManager {
  ...
  // ========================================
  // THREE와 Pane은 전역 변수로 사용 (CDN에서 로드됨)
  ...
  const setupPane = () => {
    if (typeof Tweakpane === 'undefined') {
      console.warn('Tweakpane not loaded, skipping UI setup');
      return;
    }
  ...
  // Navigation UI - 주석처리 (HTML 요소가 없으므로)
  ...
  const navigateToSlide = (targetIndex) => {
    ...
    // updateNavigationState(currentSlideIndex); // 주석처리 (HTML 요소 없음)
    gsap.fromTo(
